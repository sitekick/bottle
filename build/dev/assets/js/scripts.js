...
function standardNav() {
	...
	$("nav .level2").each( function(){
   		...
   		$(this).css('top', offset).
   		data({calcheight : y,borderw : yb, offset: offset}).
   		hide();
	});
	...
	$("nav .level1 > li").off().on({
		mouseenter: function() {_fireDropDown('down', this)},
		mouseleave: function () {_fireDropDown('up', this)}
	})
}
...
$(function(){
	...
	/* Software */	
		...
		$.getJSON('assets/data/software.json', function(data) {
				...
				let target = '.software .scrollwrap';				
				//the select field change event fires the build mechanism for the table
